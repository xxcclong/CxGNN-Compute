...
def run(head, feat, hidden, edge, node, rel, node_dst):
    ...
    nn_workload_opt = feat * hidden * head + feat * node * head + (
        feat * hidden * node * head)
...
def exec(x, ptr, idx, feat, hidden, head):
    batch = torch.load(
        "/home/huangkz/repos/CxGNN-Compute/papers100M-graphsaint.pt")
    ptr = batch['ptr'].cuda()
    idx = batch['idx'].cuda()
    ...
    res_graph = cxgc.prof(
        "spmm", "", lambda: cxgnncomp_backend.sage_mean_forward(
            x, ptr, idx, num_node_in_layer[-2]))
    print(f"nn: {res_nn[0]}, graph: {res_graph[0]}")
    print(
        f"nn: {res_nn[1]/(hidden * feat * x.shape[0])}, graph: {res_graph[1] /(x.shape[1] * idx.shape[0])}"
    )
    # tuner = cxgc.Tuner()
