...
def NeighborLstmOneByOneOP(module, ptr, idx, feat):
    ...
    for i in range(ptr.shape[0] - 1):
        ...
        if deg > 0:
            ...
            tmp = module(torch.index_select(feat, 0, comp_idx).view(deg, -1), None)
...
def NeighborLstmOP(module, ptr, idx, feat, deg_count):
    ...
    for deg, num in enumerate(deg_count):
        if num > 0 and deg > 0:
            ...
            tmp = module(torch.index_select(feat, 0, comp_idx).view(num, deg, -1), None)
...
def NeighborLstmPadOP(
    module, ptr, idx, feat, deg_count, num_center_in_batch, num_neighbor_in_batch
):
    # check
    # assert not torch.any(deg[1:] < deg[:-1])  # increasing
    ...
    output = torch.empty([num_center_node, feat.shape[-1]], device=feat.device)
    for deg, num in enumerate(deg_count):
        # print(deg, num, torch.cuda.memory_allocated()/1024/1024/1024)
        if num > 0 and deg > 0:
        ...
        if (
            accumulated_dst >= num_center_in_batch
            or accumulated_src >= num_neighbor_in_batch
            or (deg == len(deg_count) - 1 and accumulated_dst > 0)
        ):
            max_deg = deg
            comp_in = torch.zeros(
                [accumulated_dst, max_deg, feat.shape[-1]], device=feat.device
            )
            tmp_dst_cnt = 0
            for deg2 in range(min_deg, max_deg + 1):
                ...
                if num2 > 0:
                    ...
                    comp_in[tmp_dst_cnt : tmp_dst_cnt + num2, :deg2, :] = (
                        torch.index_select(feat, 0, comp_idx).view(num2, deg2, -1)
                    )
                    src_cnt += num2 * deg2
                    tmp_dst_cnt += num2
            ...
            output[dst_cnt : dst_cnt + accumulated_dst] = (
                tmp[0].view(accumulated_dst, max_deg, -1).sum(1)
            )
