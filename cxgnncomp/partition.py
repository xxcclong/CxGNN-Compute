...
def partition_2d_gpu(edge_index, num_device, rank_dst, rank_src):
    num_node = torch.max(edge_index) + 1
    num_node_per_device = num_node // num_device
    # print(edge_index.shape)
    lower_bound_dst = num_node_per_device * rank_dst
    upper_bound_dst = num_node_per_device * (rank_dst + 1) if rank_dst < num_device - 1 else num_node
    lower_bound_src = num_node_per_device * rank_src
    upper_bound_src = num_node_per_device * (rank_src + 1) if rank_src < num_device - 1 else num_node
    ...
    degree = torch.bincount(edge_index[1],
                            minlength=num_node).to(edge_index.device)
